...
from flask import Flask, render_template ,request,url_for,redirect, send_file ,session #import flask class
# <<<<<<< HEAD
...
def login():
    if request.method == 'POST':
        ...
        sql = "SELECT * FROM dr WHERE uname = %s AND pass = %s AND admin = '1'"
        val = (username,password)
        myCursor15.execute(sql,val)
        if myCursor15.rowcount == 0 :
